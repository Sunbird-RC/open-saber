...
public class RegistryServiceImpl implements RegistryService {
	...
	public void deleteEntityById(Shard shard, String userId, String uuid) throws Exception {
		...
		try (OSGraph osGraph = databaseProvider.getOSGraph()) {
			...
			VertexReader vertexReader = new VertexReader(databaseProvider, graph, configurator, uuidPropertyName,
					definitionsManager);
			...
			if (!(vertex.property(Constants.STATUS_KEYWORD).isPresent()
				...
				String index = vertex.property(Constants.TYPE_STR_JSON_LD).isPresent()
						? (String) vertex.property(Constants.TYPE_STR_JSON_LD).value()
						: null;
				...
				if (auditEnabled) {
					...
					AuditRecord auditRecord = auditService.createAuditRecord(userId, Constants.AUDIT_ACTION_DELETE,
							uuid, transactionId);
					auditRecord.setAuditInfo(auditService.createAuditInfo(Constants.AUDIT_ACTION_DELETE_OP,
							Constants.AUDIT_ACTION_DELETE, null, null, entityTypes));
	...
	public String addEntity(Shard shard, String userId, JsonNode rootNode) throws Exception {
		...
		if (persistenceEnabled) {
			...
			// if Audit enabled in configuration yml file
			if (auditEnabled) {
				AuditRecord auditRecord = auditService.createAuditRecord(userId, Constants.AUDIT_ACTION_ADD, entityId,
						transactionId);
				auditRecord.setAuditInfo(auditService.createAuditInfo(Constants.AUDIT_ACTION_ADD_OP,
						Constants.AUDIT_ACTION_ADD, null, rootNode, entityTypes));
	...
	public void updateEntity(Shard shard, String userId, String id, String jsonString) throws Exception {
		...
		try (OSGraph osGraph = databaseProvider.getOSGraph()) {
			...
			if (signatureEnabled) {
				...
				String entitySignUUID = signatureHelper.removeEntitySignature(parentEntityType,
...
//            logger.debug("Validated payload before update");
			...
			if (auditEnabled) {
				...
				AuditRecord auditRecord = auditService.createAuditRecord(userId, Constants.AUDIT_ACTION_UPDATE, id,
						transactionId);
				auditRecord.setAuditInfo(auditService.createAuditInfo(Constants.AUDIT_ACTION_UPDATE_OP,
						Constants.AUDIT_ACTION_UPDATE, readNode, mergedNode, entityTypes));
	...
	private void doUpdate(Shard shard, Graph graph, IRegistryDao registryDao, VertexReader vr, JsonNode userInputNode)
		...
		if (existingVertex != null) {
			...
			while (fieldsItr.hasNext()) {
				...
				if (!oneElement.getKey().equals(uuidPropertyName) && oneElementNode.isValueNode()
						|| oneElementNode.isArray()) {
					...
					if (oneElementNode.isArray()) {
						...
						if (null != existArrayVertex) {
							...
							VertexWriter vertexWriter = new VertexWriter(graph, shard.getDatabaseProvider(),
									uuidPropertyName);
	 ...
	 * mergeDestinationWithSourceNode method for deep copy of properties and objects
	 *
